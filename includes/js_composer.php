...
function viivue_js_composer_icomoon($settings, $value){
	...
	$input_html = '<input name="' . esc_attr($param_name) . '" class="hidden wpb_vc_param_value wpb-textinput ' . esc_attr($param_name) . ' ' . esc_attr($type) . '_field" type="text" v-model="selected.name" data-icomoon-input/>';
	
	return $acf_icomoon->viivue_icomoon_select_html($icon_array, $value, $input_html, true);
}